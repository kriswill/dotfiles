...
# a static stow file; it's generated here and linked during activation.
{
  flake.modules.darwin.direnv-nom =
    ...
    let
      ...
      # Closure diff shown after a successful build (nh-style nvd output; the
      # old kriswill.direnv-nom.diff enum was never set to anything else).
      diffCmd = "${lib.getExe pkgs.nvd} diff";
