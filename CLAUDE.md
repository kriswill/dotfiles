...
Secrets are sops-nix encrypted with the `nebula` age key (see `.sops.yaml`). Edit with `sops modules/hosts/_nebula/secrets.yaml`.

## Architecture

The flake is **dendritic**: `flake.nix`'s `outputs` is just `flake-parts.lib.mkFlake { inherit inputs; } (import-tree ./modules)`. `import-tree` (`vic/import-tree`) recursively discovers every `.nix` file under `modules/` and feeds them to flake-parts as modules — **except any path containing `/_`** (so `modules/hosts/_nebula/` is skipped by the scan and pulled in by explicit `imports`). There is no central `imports = [ ... ]` list to maintain. The host build still flows through `snowglobe-lib`.
...
- **`modules/hosts/nebula.nix`** — registers `configurations.nixos.nebula` (hostname, `cpu-vendor`, `gpu-vendors`, `firmware`, `isVM`, `stateVersion`). Its `module` imports every `flake.modules.nixos.*` plus the host-specific plain modules under `_nebula/`, applies the overlays (`nixpkgs.overlays = builtins.attrValues config.flake.overlays`), and sets `sops.defaultSopsFile`.
- **`modules/hosts/_nebula/`** — host-specific plain NixOS modules (excluded from the import-tree scan; imported explicitly by `nebula.nix`). `configuration.nix` toggles the host's `snowglobe-lib.profiles.*` (`hacker-mode`, `gaming`, `office`, `harden`, `nix-tools`, `hardware-tools`) and `snowglobe-lib.desktop.*`. `disko.nix` declares partitioning (single-disk, ext4 root, vfat ESP). `hardware-configuration.nix` is generated — don't hand-edit. `users/k/` defines the primary user (sops-managed password). `secrets.yaml` + the `*.pub` files live here too.
...
When adding shared functionality, drop a `{ flake.modules.nixos.<name> = …; }` file under `modules/nixos/` (auto-imported). When adding host-specific config, add a file under `modules/hosts/_nebula/` and list it in `modules/hosts/nebula.nix`'s `imports`. New custom packages go in `packages/` and become available as `pkgs.<name>` via the overlay.
