...
| [`docs/helium.md`](docs/helium.md) | **Helium browser** — unbranded Chromium fork; Nix install (`programs.helium`, the `helium` deferredModule merge), declarative managed policies at `/etc/chromium/policies/managed/`, force-installed extensions + built-in uBlock, profile/XDG layout, and the `helium-config` snapshot tooling (settings into the repo without stow) |
...
| [`docs/suspend.md`](docs/suspend.md) | **Suspend/sleep** — S3 vs s2idle, wake-source diagnostics, and the MSI BIOS `Wake Up Event By: OS` setting that fixed immediate wake-from-S3 (firmware was overriding all Linux wakeup controls) |
| [`docs/fastfetch.md`](docs/fastfetch.md) | **fastfetch** — logo config options, why the PNG logo needs `kitty-icat` (not native `kitty`) in ghostty, the `TERM=="screen"` image guard that broke it in tmux, and the ghostty+Hyprland+grim screenshot test harness |
| [`docs/tmux.md`](docs/tmux.md) | **tmux** — stow + Nix-module config split, the deliberate settings (`default-terminal tmux-256color`, `allow-passthrough`, vim-tmux-navigator, titles), and the graphics/passthrough caveats for in-pane images |
...
- **`packages/`** — where custom `pkgs.callPackage` derivations go (currently `dots-adopt` the dotfiles capture helper, and `wowup`). Reused by both `modules/packages.nix` and the `my-packages` overlay.
