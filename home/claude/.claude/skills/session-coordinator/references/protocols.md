...
## Context compaction (before handover)

At ~80% context, compact rather than hand off: (a) the teammate refreshes its CHECKPOINT file first — it is the memory across the compaction; (b) the coordinator sends `/compact <focus line>` naming what to preserve verbatim (role, ownership, changelist/branch, standing rules, unfinished directives, open findings by id, recipes) and what to drop (tool outputs, file dumps, closed threads), ending "re-read TEAM.md, your checkpoint and your status tail before acting"; (c) after it fires, the coordinator RE-SENDS the held directive list and gets "HELD: n/m" back. If a turn is running, the command queues and fires at the turn's end; verify it fired (context drops) rather than assume. Hand off (fresh agent from a `brief-<role>2.md` that points at the checkpoint and says "re-verify, do not inherit") only at ~90% or if compaction does not bring the number down. Four compactions in one mission held every directive by self-report and needed no re-issue; the self-report is not a measurement — the coordinator's held list is.
