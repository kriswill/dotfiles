...
// by `okf scaffold` via okf.toml `[scaffold] script`. Walks modules/, pkgs/,
// overlays/, flakes/, modules/hosts/ and the Neovim plugin specs, and writes
// one stub per component into knowledge/{modules,packages,hosts,nvim/plugins}/.
// Idempotence, --force, and the written/skipped summary are owned by the
// injected ctx.emit (flakes/okf/scaffold-api.ts); this script uses only the
...
export default async function scaffold(ctx: ScaffoldContext) {
  const { emit, titleFromSlug, clean, sentence, firstSentence, mdSafe, firstMatch } = ctx;
  const repo = ctx.root;
