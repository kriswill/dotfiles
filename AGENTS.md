...
- **Module pattern:** a module mounted into a host is active — no enable options, no `lib.mkIf` gating. Universal features (every darwin host) are plain deferred modules in `flake.modules.darwin.<name>` (see `modules/darwin/tmux.nix`); hosts blanket-import them via `builtins.attrValues config.flake.modules.darwin`. Host-selective features are dendritic files under `modules/hosts/` that merge directly into `configurations.darwin.<host>.module` (see `modules/hosts/podman-desktop.nix` for a two-host mount, `modules/hosts/k/claude-account-selector/` for a single-host one).
- **Imports:** Use `inherit` for cleaner destructuring
- **Package lists:** Use `builtins.attrValues { inherit (pkgs) ...; }` pattern
- **Options:** define options only for genuinely parameterized modules (e.g. the sub-flakes' `services.apple-container.*`); in-tree feature modules carry no toggles
...
- **Module mounting:** universal features in `flake.modules.darwin.<name>`; host-selective features under `modules/hosts/` mounting into `configurations.darwin.<host>.module`
...
│   ├── darwin/          # nix-darwin feature modules universal to all hosts (core, git, kitty, …)
│   └── hosts/           # Per-host config + host-selective feature mounts
│       ├── k.nix, mini.nix, SOC-Kris-Williams.nix   # host definitions
│       ├── podman-desktop.nix, apple-container.nix, codebase-memory-mcp.nix  # k+SOC mounts
│       ├── k/           # k-only features (claude-account-selector)
│       └── SOC-Kris-Williams/  # SOC-only features (alias-en0)
...
1. Decide where it mounts. Universal (every darwin host): a bare `<name>.nix` in `modules/darwin/` defining `flake.modules.darwin.<name>` — hosts blanket-import the whole set. Host-selective: a file under `modules/hosts/` that merges into `configurations.darwin.<host>.module` directly (one file can mount a shared module into several hosts; see `modules/hosts/podman-desktop.nix`). Single-host features live in `modules/hosts/<host>/`.
2. No enable option, no `lib.mkIf` gate — being mounted is what turns it on. Only use a directory when bundling adjacent config files.
