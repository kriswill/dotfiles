...
- **Symlinks:** Stow tree under `home/` for plain config files (see Symlinked Configs below); `config.lib.file.mkOutOfStoreSymlink` for home-manager-managed files (see oksh module)
...
│   ├── tmux/, zsh/, fastfetch/   # deployed by modules/darwin/dotfiles-stow.nix on every rebuild
├── config/              # Config files symlinked via home-manager (ghostty, karabiner, oksh, containers)
...
**Symlinked Configs (two mechanisms, both point at the live repo — editable without rebuild):**

1. **Stow tree (`home/`)** — preferred for plain config files. Each dir under `home/` is one stow package mirroring `$HOME` (e.g. `home/tmux/.config/tmux/tmux.conf`). `modules/darwin/dotfiles-stow.nix` restows every package during activation (`--no-folding`, conflicts logged + skipped per package, stale links self-healed). Adding a package = adding a directory; no nix edit needed. Capture an existing file with `dots-adopt <pkg> <relpath-under-$HOME>`; pull live edits of an already-tracked file back with `stow -d ~/src/dotfiles/home -t ~ --no-folding --adopt <pkg>` (overwrites the repo copy). Files that must embed /nix/store paths (e.g. tmux `plugins.conf`) are generated by the per-feature darwin module and linked in activation instead.
2. **`config/` + `config.lib.file.mkOutOfStoreSymlink`** — for configs still managed by a home-manager module (ghostty, karabiner, oksh).
