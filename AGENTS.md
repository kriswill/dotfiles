...
- **Symlinks:** Stow tree under `home/` for plain config files (see Symlinked Configs below); `config.lib.file.mkOutOfStoreSymlink` for home-manager-managed files (see podman-desktop module)
...
│   ├── tmux/, zsh/, fastfetch/, ghostty/, karabiner/, glow/, oksh/, starship/
│   │                    #   …deployed by modules/darwin/dotfiles-stow.nix on every rebuild
├── config/              # Config files symlinked via home-manager (containers/podman-desktop)
...
2. **`config/` + `config.lib.file.mkOutOfStoreSymlink`** — for configs still managed by a home-manager module (containers/podman-desktop).
