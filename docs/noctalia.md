...
**`noctalia-config`** (`pkgs/noctalia-config.nix`, on `k`'s PATH):
...
- **Editing `settings.toml` IN PLACE while Noctalia runs corrupts it — write
  ...
  *same directory* and `mv -f` it over the target (atomic rename → the watcher
  ...
  pkgs/tomato.nix — Rust `toml_edit`, comment/format-preserving), but because
...
- **nixpkgs ships a competing `programs.noctalia` module (2026-08-02).**
  ...
  because it is pinned in lockstep with the installed binary (see
  `knowledge/decisions/noctalia-flake-module-over-nixpkgs.md`).
